...
#from diffefwi.plots import *   #lina注释
#from diffefwi.fwi import fwi_loop  #lina注释
...
class DenoisingDiffusionProbabilisticModel:
    ...
    def sample(self, model, image_size, batch_size=8, channels=3, batches=False):
        ...
        return self.p_sample_loop(model, shape=(batch_size, channels, image_size[0],image_size[1]), batches=batches)
    ...
    def fwi_sample_xiao(self, model, shape, init_time_step, vp_img):
        """
        Perform data-fidelity guided (from EFWI) posterior diffusion sampling.
        ...
        batch_size = shape[0]
        device = next(model.parameters()).device
        for i in reversed(range(0, self.timesteps-init_time_step)):
            if True:
                img = vp_img
                if self.patch:
                    _, img_mean = self.p_sample_wf(model, img, torch.full((batch_size,), i, device=device, dtype=torch.long))
                    ...
                    _, img_mean = self.p_sample_wf(
                        model, 
                        torch.from_numpy(resize(img.detach().cpu().numpy(), (img.shape[0],img.shape[1],img.shape[2],img.shape[3]))).to(device), 
                        torch.full((batch_size,), i, device=device, dtype=torch.long)
                    )
                img_mean =  torch.from_numpy(resize(img_mean.detach().cpu().numpy(), (img.shape[0],img.shape[1],vp_img.shape[2],vp_img.shape[3]))).to(device)
        return img_mean        
    ...
    def fwi_sample_Lina(self, model, shape, init_time_step, vp_img):
        """
        Perform progressive reverse diffusion sampling.

        :param model: PyTorch model.
        :param shape: an [N x C x H x W] array.
        :param init_time_step: Initial time step for sampling.
        :param vp_img: Initial input tensor.
        :return: final denoised mean image.
        """
        ...
        # 只初始化一次，后续逐步更新
        img = vp_img.to(device)

        for i in reversed(range(0, self.timesteps - init_time_step)):
            t = torch.full((batch_size,), i, device=device, dtype=torch.long)

            if self.patch:
                # pred_img -> 下一步输入; img_mean -> 当前步去噪均值
                img, img_mean = self.p_sample_wf(model, img, t)
            else:
                ...
                pred_img, pred_mean = self.p_sample_wf(model, img_in, t)

                img = torch.from_numpy(
                    resize(
                        pred_img.detach().cpu().numpy(),
                        (img.shape[0], img.shape[1], vp_img.shape[2], vp_img.shape[3])
                    )
                ).to(device)

                img_mean = torch.from_numpy(
                    resize(
            ...
            img_mean = torch.clamp(img_mean, min=-1., max=1.)
